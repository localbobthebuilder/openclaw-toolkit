...
if (-not $usingPowerShellCore -and $null -ne $pwshCommand) {
    ...
    Write-Host "INFO: Next time, launch via cmd\run-bootstrap.cmd or run:" -ForegroundColor Yellow
...
foreach ($workspace in @(Get-ToolkitSharedWorkspaceList -Config $config)) {
    ...
    foreach ($workspaceAgentId in @($workspace.agents)) {
        ...
        if ($null -ne $workspaceAgent -and (Test-ToolkitAgentEnabled -AgentConfig $workspaceAgent) -and (Test-ToolkitAgentAssigned -Config $config -AgentConfig $workspaceAgent)) {
